 ...
 * 前缀（参考中有此前缀） -p, --threads <int>            使用的工作线程数（默认
 * 1） -h, --help                   打印帮助信息并退出
 *
 * 输出格式为 TSV，共 7 列（带表头）：
 ...
 *      工作线程解析每条记录，并直接更新全局转换统计表
 * globalConvTable，同时更新每个染色体的当前最大坐标 globalMaxPos。
 *   2. 另有一个 flusher 线程定期检查
 * globalConvTable，对于某染色体上比对坐标远小于该染色体当前最大坐标（减去窗口值）的记录，
 *      认为已完成统计，直接将其输出到目标文件并从内存中删除，以降低内存占用。
 *   3. 正负链处理：对于负链比对，程序先对 read
 * 碱基取互补，同时参考碱基也取互补，然后按互补后的结果比较转换情况。
 */
...
int main(int argc, char *argv[]) {
  ...
  // 由工作线程直接更新全局转换统计表 globalConvTable 和 globalMaxPos，
  // 因此不再使用各线程的局部表
